...
class SimpleExcelService:
    ...
    def get_values(self, ws, min_row, min_col):
        # 指定したセル範囲を取得する ←どういう形式にしたら使いやすいだろう
        result = np.array([row for row in self.ws.iter_rows(
            min_row=min_row, min_col=min_col, values_only=True)])
        return result
